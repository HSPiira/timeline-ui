...
function WorkflowsPage() {
  ...
  const fetchWorkflows = async () => {
    ...
    try {
      ...
      if (apiError) {
        // Check various error properties for permission-related messages
        ...
        setWorkflows(data)
        // Extract unique event types from triggers
        const types = [...new Set(data.map((w: Workflow) => {
          // Assuming trigger has event_type field
  ...
  const handleCreateWorkflow = async (workflowData: WorkflowCreate) => {
    ...
    try {
      ...
      if (apiError) {
        const errorMsg = typeof apiError === 'object' && 'message' in apiError ? (apiError as any).message : 'Failed to create workflow'
        setError(errorMsg)
        return false
      }
  ...
  const handleDeleteWorkflow = async (workflowId: string) => {
    ...
    try {
      ...
      if (apiError) {
        const errorMsg = typeof apiError === 'object' && 'message' in apiError ? (apiError as any).message : 'Failed to delete workflow'
        setError(errorMsg)
      } else {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left py-2 px-2.5 font-medium text-muted-foreground text-sm">Trigger Event Type</th>
                <th className="text-left py-2 px-2.5 font-medium text-muted-foreground text-sm">Status</th>
                <th className="text-left py-2 px-2.5 font-medium text-muted-foreground text-sm">Actions Count</th>
              ...
              {filteredWorkflows.map((workflow: Workflow) => {
                ...
                return (
                  <tr key={workflow.id} className="border-b border-border hover:bg-muted/50 transition-colors">
                    ...
                    <td className="py-2 px-2.5">
                      <span className="text-muted-foreground text-sm bg-secondary px-1.5 py-0.5 rounded">{triggerEventType}</span>
                    </td>
                    <td className="py-2 px-2.5">
                    ...
                    <td className="py-2 px-2.5">
                      <span className="text-muted-foreground text-sm">{actionsCount} action{actionsCount !== 1 ? 's' : ''}</span>
                    </td>
